...
namespace NHibernate.Test.TypesTest
{
	...
	public abstract class GenericTypeFixtureBase<TProperty, TType> : TestCase where TType : IType
	{
		...
		/// </summary>
		protected virtual IEnumerable<Expression<Func<TProperty, object>>> PropertiesToTest => Enumerable.Empty<Expression<Func<TProperty, object>>>();

		/// <summary>
		/// Methods of <see cref="TType"/> which should be queryable using LINQ
		/// </summary>
		protected virtual IEnumerable<Expression<Func<TProperty, object>>> MethodsToTest => Enumerable.Empty<Expression<Func<TProperty, object>>>();
		...
		public virtual void CanPersist()
		{
			...
			using (var trans = session.BeginTransaction())
			{
				foreach (var expectedValue in expectedValues)
				{
					...
					Assert.That(entity.TestProperty, Is.EqualTo(AdjustValue(expectedValue.Value)));
				}
			}
		}
